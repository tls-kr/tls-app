...
(: this is abandoned as of 2021-10-12, we loop through the swl class on a page instead, see show_swl_for_line -> lrh:format-swl :)
...
for $sh in $h/preceding-sibling::tei:seg[position()<4] return lrh:proc-seg($sh, map{"punc" : true(), "textid" : $textid}),
        ...
        if ($tr) then (for $t in $tr[position()<4] 
          let $ai := ltr:get-translation-css($t)
          , $aitit := if (string-length($ai) > 0) then "Translation generated by AI" else ""
         return (<br/>,' - ', <span class="{$ai}" title="{$aitit}">{string-join($t, '')}</span> ) ) else ()}</div>
</div>
}
