...
//  var location = window.location.search;
  ...
  // Persist the slot choice FIRST via the fast save-only endpoint.
  // get_tr_for_page and ltr_reload_selector each run ltr:get-translations
  // (30-50s, 3 full-collection scans) in the same XQuery transaction as
  // the save, so if the user navigates away during the request the HTTP
  // cancellation rolls back the save. save_slot_config does only the
  // persist step and commits in <100ms, so the choice sticks even if the
  // user clicks "next page" immediately after selecting.
  var textid = (location || '').split('_')[0];
  $.ajax({
    type: "GET",
    url: "api/save_slot_config?textid=" + encodeURIComponent(textid) +
         "&slot=" + encodeURIComponent(slot) +
         "&content-id=" + encodeURIComponent(myid)
  });
   $.ajax({
   type : "GET",
   dataType : "html",
   ...
   success : function(resp){
    ...
    for (var prop in obj) {
     ...
     aiclass = obj[prop][2];
     $(line_id).removeClass();
     $(line_id).addClass("tr " + aiclass);
     $(line_id).parent().attr('lang', obj[prop][1]);
     if (aiclass == 'ai ') {
         $(line_id).parent().attr('title', 'Translation generated by AI');
     } else {
         $(line_id).parent().attr('title', '');
     }
     $(line_id).html(obj[prop][0]);
    };
    // Refresh the dropdown header with the new selection. Slow (rebuilds
    // the submenu via ltr:get-translations) but non-critical: the save
