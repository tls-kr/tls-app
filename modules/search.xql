...
{
if (matches($queryStr, '^[A-Za-z]+')) then 
     <ul>{
     for $hit in collection($config:tls-data-root)//tei:div[@type='syn-func']
     let $head := data($hit/tei:head)
     where starts-with($head, $queryStr)
     order by $head
     return <li><a href="syn-func.html?uuid={$hit/@xml:id/string()}">{$hit/tei:head}</a></li>
     }</ul>
 else
...
let $st :=  try {if (string-length($type) > 0) then map:get($config:search-map, $map?search-type) || "/" || map:get($config:lmap, $type) else map:get($config:search-map, $map?search-type)} catch * {'texts'}
return
