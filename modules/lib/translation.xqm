...
import module namespace ltp="http://hxwd.org/lib/template" at "template.xqm";
...
(: 2025-11-12  positional arguments are repurposed, not all used  :)
declare function ltr:new-ai-translation(
  $lang as xs:string, 
  $txtid as xs:string, 
  $vendor as xs:string, 
...
, $genm := map:merge( for $s in $segs 
           let $resp := normalize-space($s/@generatedBy)
           group by $resp
           return 
           map:entry($resp, count($s)))
, $gens := <ul>{for $k in map:keys($genm)         
           let $cnt := $genm?($k)
           order by $cnt descending
           return 
           <li><b class="ml-2">{$k}</b> <span class="ml-2">{$cnt}</span></li>}</ul>
...
<div class="col">{(
  ...
  ,lrh:display-row(map{"col2" : "Translators/Operators", 
                      ...
                      "col3" : $trs})
  ,lrh:display-row(map{"col2" : "Generated By", 
                      "col2-tit" : "This shows who is responsible for the content, especially for AI translations", 
                      "col3" : $gens})
  ...
  <button type="button" class="btn btn-secondary" onclick="goto_translation_seg('{$trid}', 'first')" title="Go to first translated line">←</button> 
   <div class="dropdown" id="{$slot}" data-trid="{$trid}">
    ...
    <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
        ...
        <a class="dropdown-item" onclick="show_dialog('new-ai-trans-dialog', {{'textid': '{$textid}', 'slot': '{$slot}'}})" href="#"> <button class="btn btn-warning" type="button">Request new AI translation</button></a>
        )
         else ()
        }
