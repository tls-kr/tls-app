...
function app:browse($node as node()*, $model as map(*), $type as xs:string?, $filter as xs:string?, $mode as xs:string?)
{
    ...
    else if ($type = "basic-texts") then i18n:display(map{'id': 'basic-texts'})
...
{   if (string-length($edit) > 0) then () else
    ...
    , $crit := for $p in collection($config:tls-data-root||"/concepts")//tei:div[@type="old-chinese-criteria" and contains(., " "|| $char || " ")] 
      return $p
